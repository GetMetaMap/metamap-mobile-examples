...
const index = require('./index-e7aa7c0d.js');
const cubicBezier = require('./cubic-bezier-04b3abd3.js');
    ...
    componentDidLoad() {
        {
            rIC(() => {
                ...
                if (!config.config.getBoolean('_testing')) {
                    new Promise(function (resolve) { resolve(require('./tap-click-84165356.js')); }).then(module => module.startTapClick(config.config));
                }
                if (config.config.getBoolean('statusTap', isHybrid)) {
                    new Promise(function (resolve) { resolve(require('./status-tap-9f8bcd5c.js')); }).then(module => module.startStatusTap());
                }
                ...
                if (config.config.getBoolean('hardwareBackButton', isHybrid)) {
                    new Promise(function (resolve) { resolve(require('./hardware-back-button-153e2eed.js')); }).then(module => module.startHardwareBackButton());
                }
...
const handleContentScroll = (scrollEl, mainHeaderIndex, scrollHeaderIndex, remainingHeight = 0) => {
    core.readTask(() => {
        ...
        const borderOpacity = helpers.clamp(0, (scrollTop - remainingHeight) / lastMainToolbar.el.clientHeight, 1);
        const maxOpacity = 1;
...
const RouterOutlet = class {
    ...
    async connectedCallback() {
        this.gesture = (await new Promise(function (resolve) { resolve(require('./swipe-back-baf527a3.js')); })).createSwipeBackGesture(this.el, () => !!this.swipeHandler && this.swipeHandler.canStart() && this.animationEnabled, () => this.swipeHandler && this.swipeHandler.onStart(), step => this.ani && this.ani.progressStep(step), (shouldComplete, step, dur) => {
            if (this.ani) {
